    ...
    if request.method == "GET":
        ...
        # 作業報告書
            ...
            try:
                ...
                if target == "project_name":
                    ...
                    site_work_time = StringTime(data["site_work_time"]).time
                    print(f"site_work_time:{site_work_time}")
                    print(type(site_work_time))
                    ...
                    start_time=StringTime(data["start_time"]).time
                    end_time=StringTime(data["end_time"]).time
                    break1_time=StringTime(data["break1_time"]).float_time
                    break2_time=StringTime(data["break2_time"]).float_time
        ...
        # 設定値
            ...
            try:
                ...
                start_time=StringTime(data["start_time"]).time
                end_time=StringTime(data["end_time"]).time
                break1_time=StringTime(data["break1_time"]).time
                break2_time=StringTime(data["break2_time"]).time
    ...
    try:
        ...
        for work_date,work_detail in work_details.items():
            if not work_date.strftime("%a") in holiday_week:
                ...
                work_detail.break1 = StringTime(report_settings.break1).float_time
                work_detail.break2 = StringTime(report_settings.break2).float_time
...
def export_excel(request):
    ...
    if year and month:
        ...
        file_name = f"【自社用】作業報告書_{user_id}_{user_name}_{report_period.year}年{report_period.month}月.xlsm"
        ...
        for work_detail in work_details.values():
            ...
            sheet.cell(row=12+count,column=5).value = StringTime(work_detail.start_time).serial_time
            ...
            sheet.cell(row=12+count,column=9).value = StringTime(work_detail.end_time).serial_time
            ...
            sheet.cell(row=12+count,column=13).value = StringTime(work_detail.break1_time).serial_time
            ...
            sheet.cell(row=12+count,column=16).value = StringTime(work_detail.break2_time).serial_time
        ...
        sheet.cell(row=44,column=19).value = StringTime(work_report.site_work_time).serial_time
